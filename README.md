...
Scan a barcode on a PM85 handheld → print it on a paired BIXOLON Bluetooth printer, automatically.

- **Scanner:** PM85 SDK (`device.sdk`)
...
| `app/sdk/device.sdk.jar` | PM85 device SDK |
...
4. Sync → Run on a PM85 device.

## Usage

1. Pair the BIXOLON printer in Android Bluetooth settings.
2. Launch the app → **Refresh** → pick paired device → **Connect**.
3. Pull the PM85 hardware trigger (or tap **Scan ON**) → the scanned barcode prints automatically.
